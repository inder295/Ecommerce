...
export const AdminCategories = () => {
   ...
   return (
     (<div className="ml-64 p-4">
       {
      isCategoryFetching ? <div>Loading... </div>: (
      <div >
        ...
        <table className="min-w-full border border-gray-200 divide-y divide-gray-200 ">
          ...
          </thead>
          {
            categories.map((category,index)=>(
                <tbody className="divide-y divide-gray-200 cursor-pointer" key={index}>
                    <tr className="hover:bg-gray-50" >
                      <td className="px-4 py-2 text-sm w-40 truncate">{category.id}</td>
                      <td className="px-4 py-2 text-sm w-40 truncate">{category.name}</td>
                      <td className="px-4 py-2 text-sm w-100 break-all">{category.description}</td>
