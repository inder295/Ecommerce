...
export const OrderDetailsByUser = () => {
    ...
    const { getOrderByIdForAdmin, fetchingAdminOrderById,adminOrderDetailsById}=useOrders();
       ...
       <div className=''>
           ...
           <div className='grid grid-cols-5 gap-4 p-12 my-12 pt-2 '>
              <div className='bg-slate-100 col-span-3  p-4 rounded-lg'>
                  ...
                  {
                    adminOrderDetailsById?.orderItem && adminOrderDetailsById.orderItem.length > 0 ? (
              ...
              <div className='bg-slate-100 rounded-md p-4  w-96'>
                  <p className='text-xl font-bold '>Address</p>
                   <div className='my-4'>
                      ...
                      <div className='col-start-4 col-end-6 '>
                        ...
                        <p className=''>Payment Method : {adminOrderDetailsById?.paymentMethod}</p>
                        <p className=''>Payment Status : {adminOrderDetailsById?.paymentStatus}</p>
                      </div>
               ...
               <div className=' col-span-2 col-end-3'>
                <button className='m-2 p-2 px-4 bg-black font-bold text-white rounded hover:bg-gray-800 '>Invoice</button>
                <button className='m-2 p-2 px-4 bg-black font-bold text-white rounded hover:bg-gray-800' onClick={()=>setModal(true)}>Order Status </button>
                {modal && <OrdersStatusModal closeModal={handleModal} />}
            </div>
             
              <div className='bg-slate-100 col-start-4 col-end-6 p-4 rounded-md w-96 '>
                  <p className='font-semibold'>Grand Total: ${adminOrderDetailsById?.grandTotal}</p>
                  <p className='font-semibold'>Order Status: {adminOrderDetailsById?.orderStatus}</p>
