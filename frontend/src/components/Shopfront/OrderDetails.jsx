          ...
          {userOrdersDetails.map((order) => {
            return (
              ...
              >
                <Link
                  to={'/orders/' + order.id}
                  class="bg-neutral-primary-soft block max-w-sm p-6 border border-default rounded-base shadow-xs hover:bg-neutral-secondary-medium hover:shadow-md"
                >
                  <h5 class="mb-3 text-2xl font-semibold tracking-tight text-heading leading-8 line-clamp-1">
                    OrderId : {order.id}
                  </h5>
                  ...
                  <p>
                    GrandTotal: <span> ${order.grandTotal}</span>{' '}
                  </p>
